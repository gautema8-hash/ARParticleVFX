...
export async function renderAdmin(app) {
  ...
  async function renderUsers(keyword = '') {
    ...
    panel.innerHTML = `<div class="admin-searchbar admin-search-panel"><span class="admin-search-icon">⌕</span><input id="admin-user-keyword" placeholder="搜索邮箱 / 用户名 / 昵称" value="${keyword}"><button class="btn btn-primary" id="admin-user-search" type="button">智能检索</button><button class="btn" id="admin-user-clear" type="button">清空</button></div><div class="table-wrap"><table class="compare"><thead><tr><th>ID</th><th>用户名</th><th>昵称</th><th>邮箱</th><th>档位</th><th>角色</th><th>操作</th></tr></thead><tbody>
  ...
  async function renderLogs(keyword = '') { panel.innerHTML='<p class="muted">加载中…</p>'; const list=await api.adminLoginLogs(keyword); panel.innerHTML=`<div class="card admin-log-tip">仅展示最近 7 天日志，系统每天自动逻辑清理过期记录。</div><div class="feedback-filter-panel admin-search-panel"><span class="admin-search-icon">⌕</span><input id="log-search" placeholder="搜索用户名 / 昵称 / 邮箱" value="${keyword}"><button class="btn btn-primary feedback-query-btn" id="log-query" type="button">智能检索</button></div><div class="table-wrap"><table class="compare"><thead><tr><th>用户名</th><th>昵称</th><th>邮箱</th><th>操作</th><th>时间</th><th>IP</th><th>地址识别</th><th>设备</th><th>浏览器</th><th>系统</th><th>结果</th></tr></thead><tbody>${list.map(l=>`<tr><td>${l.username||'-'}</td><td>${l.nickname||'-'}</td><td>${l.email||'-'}</td><td>${l.operation}</td><td>${(l.loginTime||'').replace('T',' ')}</td><td>${l.ip||'-'}</td><td>${l.address||'-'}</td><td>${l.deviceType||'-'}</td><td>${l.browser||'-'}</td><td>${l.os||'-'}</td><td>${l.success===1?'成功':'失败'}</td></tr>`).join('')}</tbody></table></div>`; panel.querySelector('#log-query').addEventListener('click',()=>renderLogs(panel.querySelector('#log-search').value.trim())); }
  ...
  async function renderKnowledge(filters = {}) { panel.innerHTML='<p class="muted">加载中…</p>'; const list=await api.adminKnowledge(filters); panel.innerHTML=`<div class="card knowledge-editor"><h3>发布/编辑科技指南</h3><div class="form-row"><label>标题</label><input id="kn-title" placeholder="文章标题"></div><div class="form-row"><label>分类</label><input id="kn-category" placeholder="Three.js / WebAR / 性能优化"></div><div class="form-row"><label>摘要</label><input id="kn-summary" placeholder="文章摘要"></div><div class="form-row"><label>封面图片</label><input id="kn-image" type="file" accept="image/png,image/jpeg,image/webp"></div><div class="form-row"><label>正文</label><textarea id="kn-content" rows="6" placeholder="支持纯文本与换行"></textarea></div><button class="btn btn-primary" id="kn-create" type="button">保存文章（草稿）</button></div><div class="feedback-filter-panel"><input id="kn-search-title" placeholder="按标题查询" value="${filters.title||''}"><input id="kn-search-category" placeholder="按分类查询" value="${filters.category||''}"><select id="kn-search-status"><option value="all">全部状态</option><option value="0">草稿</option><option value="1">已发布</option></select><button class="btn btn-primary" id="kn-query" type="button">查询文章</button></div><div class="table-wrap"><table class="compare"><thead><tr><th>ID</th><th>标题</th><th>分类</th><th>状态</th><th>创建时间</th><th>操作</th></tr></thead><tbody>${list.map(x=>`<tr><td>${x.id}</td><td>${x.title||''}</td><td>${x.category||''}</td><td>${x.status===1?'已发布':'草稿'}</td><td>${(x.createTime||'').replace('T',' ')}</td><td><button class="btn" data-kn-edit="${x.id}" type="button">编辑</button><button class="btn" data-kn-delete="${x.id}" type="button">删除</button></td></tr>`).join('')}</tbody></table></div>`; let imageBase64=''; panel.querySelector('#kn-image').addEventListener('change',e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>imageBase64=r.result;r.readAsDataURL(f);}); panel.querySelector('#kn-query').addEventListener('click',()=>renderKnowledge({title:panel.querySelector('#kn-search-title').value.trim(),category:panel.querySelector('#kn-search-category').value.trim(),status:panel.querySelector('#kn-search-status').value})); panel.querySelector('#kn-create').addEventListener('click',async()=>{await api.createKnowledge({title:panel.querySelector('#kn-title').value,category:panel.querySelector('#kn-category').value,summary:panel.querySelector('#kn-summary').value,content:panel.querySelector('#kn-content').value,imageBase64,status:0});showToast('文章已保存');renderKnowledge(filters);}); panel.querySelectorAll('[data-kn-edit]').forEach(b=>b.addEventListener('click',()=>{const x=list.find(v=>String(v.id)===b.dataset.knEdit);if(!x)return;panel.querySelector('#kn-title').value=x.title||'';panel.querySelector('#kn-category').value=x.category||'';panel.querySelector('#kn-summary').value=x.summary||'';panel.querySelector('#kn-content').value=x.content||'';imageBase64=x.imageBase64||'';panel.querySelector('#kn-create').textContent='保存修改';panel.querySelector('#kn-create').onclick=async()=>{await api.updateKnowledge(x.id,{title:panel.querySelector('#kn-title').value,category:panel.querySelector('#kn-category').value,summary:panel.querySelector('#kn-summary').value,content:panel.querySelector('#kn-content').value,imageBase64,status:x.status});showToast('文章已更新');renderKnowledge(filters);};})); panel.querySelectorAll('[data-kn-delete]').forEach(b=>b.addEventListener('click',async()=>{if(confirm('确认逻辑删除文章？')){await api.deleteKnowledge(b.dataset.knDelete);renderKnowledge(filters);}})); }

  async function renderOrders(keyword = '') {
    panel.innerHTML = '<p class="muted">加载中…</p>';
    const list = await api.adminOrders(keyword);
    ...
    panel.innerHTML = stats + `<div class="feedback-filter-panel"><input id="order-search" placeholder="通过用户邮箱 / 订单号查询" value="${keyword}"><button class="btn btn-primary" id="order-query" type="button">查询订单</button></div><div class="table-wrap"><table class="compare"><thead><tr><th>ID</th><th>订单号</th><th>用户邮箱</th><th>类型</th><th>金额</th><th>状态</th><th>时间</th></tr></thead><tbody>
      ${list.map(o => `<tr><td>${o.id}</td><td>${o.orderNo}</td><td>${o.userEmail||'-'}</td><td>${o.orderType === 1 ? '会员' : '特效'}</td><td>¥${o.amount}</td><td>${STATUS_TXT[o.status] ?? o.status}</td><td>${(o.createTime || '').replace('T', ' ')}</td></tr>`).join('')}
    </tbody></table></div>`;
    panel.querySelector('#order-query').addEventListener('click',()=>renderOrders(panel.querySelector('#order-search').value.trim()));
  }

  async function renderFeedback(filters = {type:'all',name:'',contact:'',status:'all'}) { panel.innerHTML='<p class="muted">加载中…</p>'; const list=await api.adminFeedbacks(filters); panel.innerHTML=`<div class="feedback-filter-panel"><select id="feedback-type"><option value="all">查询全部</option><option value="feedback">问题反馈</option><option value="custom">定制需求</option></select><input id="feedback-name" placeholder="姓名" value="${filters.name||''}"><input id="feedback-contact" placeholder="联系方式" value="${filters.contact||''}"><select id="feedback-status"><option value="all">全部状态</option><option value="0">待处理</option><option value="1">已处理</option></select><button class="btn btn-primary feedback-query-btn" id="feedback-query" type="button">查询反馈</button></div><div class="table-wrap"><table class="compare"><thead><tr><th>ID</th><th>姓名</th><th>联系方式</th><th>类型</th><th>问题描述</th><th>创建时间</th><th>处理时间</th><th>状态</th><th>操作</th></tr></thead><tbody>${list.map(f=>`<tr><td>${f.id}</td><td>${f.name}</td><td>${f.contact}</td><td>${f.type||'-'}</td><td>${f.description}</td><td>${(f.createTime||'').replace('T',' ')||'-'}</td><td>${(f.processTime||'').replace('T',' ')||'-'}</td><td>${f.status===0?'待处理':'已处理'}</td><td><button class="btn" data-feedback-status="${f.id}" type="button">标记处理</button><button class="btn" data-feedback-delete="${f.id}" type="button">删除</button></td></tr>`).join('')}</tbody></table></div>`; panel.querySelector('#feedback-type').value=filters.type||'all'; panel.querySelector('#feedback-status').value=filters.status||'all'; panel.querySelector('#feedback-query').addEventListener('click',()=>renderFeedback({type:panel.querySelector('#feedback-type').value,name:panel.querySelector('#feedback-name').value.trim(),contact:panel.querySelector('#feedback-contact').value.trim(),status:panel.querySelector('#feedback-status').value})); panel.querySelectorAll('[data-feedback-status]').forEach(b=>b.addEventListener('click',async()=>{await api.updateFeedbackStatus(b.dataset.feedbackStatus,1);renderFeedback(filters);})); panel.querySelectorAll('[data-feedback-delete]').forEach(b=>b.addEventListener('click',async()=>{if(confirm('确认逻辑删除？')){await api.deleteFeedback(b.dataset.feedbackDelete);renderFeedback(filters);}})); }

  async function renderEffects(keyword = '') {
    panel.innerHTML = '<p class="muted">加载中…</p>';
    const list = await api.adminEffects(keyword);
    panel.innerHTML = `<div class="feedback-filter-panel"><input id="effect-search" placeholder="通过用户邮箱 / 特效名称 / 编码查询" value="${keyword}"><button class="btn btn-primary" id="effect-query" type="button">查询特效</button></div><div class="table-wrap"><table class="compare"><thead><tr><th>ID</th><th>编码</th><th>名称</th><th>用户邮箱</th><th>粒子标签</th><th>创建时间</th><th>上架时间</th><th>下架时间</th><th>状态</th><th>操作</th></tr></thead><tbody>
      ${list.map(e => `<tr><td>${e.id}</td><td>${e.effectCode}</td><td>${e.effectName}</td><td>${e.publisherEmail||'xuyangtogether@163.com'}</td><td>${e.tags || '-'}</td><td>${(e.createTime||'').replace('T',' ')}</td><td>${(e.publishTime||'').replace('T',' ')||'-'}</td><td>${(e.offlineTime||'').replace('T',' ')||'-'}</td><td>${e.status === 1 ? '上架' : '下架'}</td>
    ...
    panel.querySelector('#effect-query').addEventListener('click',()=>renderEffects(panel.querySelector('#effect-search').value.trim()));
  ...
  const knowledgeObserver = new MutationObserver(() => { panel.querySelectorAll('[data-kn-edit]').forEach(edit => { const row=edit.closest('tr'); if(!row || row.querySelector('[data-kn-toggle-runtime]')) return; const button=document.createElement('button'); button.className='btn'; button.dataset.knToggleRuntime='1'; button.type='button'; button.textContent='发布/转草稿'; button.onclick=async()=>{ const list=await api.adminKnowledge(); const item=list.find(x=>String(x.id)===String(edit.dataset.knEdit)); if(!item)return; await api.updateKnowledge(item.id,{title:item.title,category:item.category,summary:item.summary||'',content:item.content||'',imageBase64:item.imageBase64||'',status:item.status===1?0:1}); showToast(item.status===1?'已转为草稿':'已发布'); renderKnowledge(); }; row.lastElementChild.insertBefore(button,row.lastElementChild.lastElementChild); }); }); knowledgeObserver.observe(panel,{childList:true,subtree:true});
  panel.addEventListener('click', (event) => { const edit=event.target.closest('[data-kn-edit]'); if(!edit)return; panel.querySelectorAll('[data-kn-edit]').forEach(other=>{ other.disabled=other!==edit; other.title=other===edit?'当前正在编辑':'请先完成当前文章编辑'; }); });
