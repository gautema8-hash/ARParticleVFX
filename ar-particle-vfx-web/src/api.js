...
export const api = {
  ...
  adminFeedbacks: (filters = {}) => { const q = new URLSearchParams({ type: filters.type || 'all', name: filters.name || '', contact: filters.contact || '' }); if (filters.status !== undefined && filters.status !== '' && filters.status !== 'all') q.set('status', filters.status); return request(`/admin/feedbacks?${q}`); },
  ...
  adminKnowledge: (filters = {}) => { const q = new URLSearchParams(); if(filters.title)q.set('title',filters.title); if(filters.category)q.set('category',filters.category); if(filters.status!==undefined&&filters.status!==''&&filters.status!=='all')q.set('status',filters.status); return request(`/admin/knowledge${q.toString()?'?'+q:''}`); },
  ...
  adminLoginLogs: (keyword = '') => request(`/admin/login-logs${keyword ? '?keyword=' + encodeURIComponent(keyword) : ''}`),
  sendTechnicalMail: (payload) => request('/admin/technical-mail', { method: 'POST', body: payload }),
  adminOrders: (keyword = '') => request(`/admin/orders${keyword ? '?keyword=' + encodeURIComponent(keyword) : ''}`),
  adminEffects: (keyword = '') => request(`/admin/effects${keyword ? '?keyword=' + encodeURIComponent(keyword) : ''}`),
