...
- **12 款普通粒子特效**（雪花 / 雨滴 / 花瓣 / 樱花 / 蝴蝶 / 游鱼 / 飞鸟 / 萤火虫 / 科技点阵 / 粒子波 / 烟花 / 星云）：自包含 Canvas 2D 引擎，详情页 **iframe 实时预览** + **一键导出单文件 HTML**（零依赖，双击即开）。
...
└── src/
    ...
    │   └── particleEffects.js  # 12 款普通粒子引擎（自包含 HTML）
    ├── tools/                  # 代码压缩 / 颜色 / 参数生成
    ├── lib/toast.js            # 轻提示
...
- **普通粒子导出**：`particleEffects.js` 生成的 HTML 为自包含、零依赖，双击即可运行。
